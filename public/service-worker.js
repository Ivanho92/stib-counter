...
const CACHED_FILES = [
    ...
    `${BASE_URL}/favicon256.png`,
    `${BASE_URL}/logo-stib.png`,
...
self.addEventListener("install", (event) => {
    ...
    ENABLE_LOGS && (console.log(""), console.groupCollapsed(`${CACHE_NAME} INSTALLED`), console.log(event), console.groupEnd());
});
...
self.addEventListener("activate", (event) => {
    ...
    ENABLE_LOGS && (console.log(""), console.groupCollapsed(`${CACHE_NAME} ACTIVATED`), console.log(event), console.groupEnd());
});

self.addEventListener("fetch", (event) => {
    ENABLE_LOGS &&
        (console.log(""),
        console.groupCollapsed(`${CACHE_NAME} FETCHING: ${event.request.url} (mode: ${event.request.mode})`),
        console.log(event),
        console.log(`URL (event.request.url): %c${event.request.url}`, "background-color: blue"),
        console.log(`Mode (event.request.mode): %c${event.request.mode}`, "background-color: blue"),
        console.groupEnd());
